...
from presentations_app.mcp import http_handler as http_handler_mod  # noqa: E402
...
def test_tools_list_matches_the_documented_surface(client):
    ...
    assert names == {
        "create_presentation", "create_presentation_from_file", "update_presentation",
        "delete_presentation", "list_presentations", "show_image", "export_presentation_to_image",
...
def test_create_presentation_from_file_html_is_stored_intact(client, store, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    original = "<html><body><h1>Report</h1><p>Generated by a script.</p></body></html>"
    f = tmp_path / "report.html"
    f.write_text(original)

    resp = _call(client, "create_presentation_from_file", {"path": str(f), "id": "from-file-html"})
    body = resp.json()
    assert body["result"]["isError"] is False
    assert store.get("from-file-html").html == original


def test_create_presentation_from_file_unknown_extension_is_rejected(client, store, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    f = tmp_path / "data.txt"
    f.write_text("hello")

    resp = _call(client, "create_presentation_from_file", {"path": str(f), "id": "from-file-txt"})
    body = resp.json()
    assert body["result"]["isError"] is True
    assert ".txt" in body["result"]["content"][0]["text"]
    assert store.get("from-file-txt") is None


def test_create_presentation_from_file_relative_path_is_rejected(client, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    resp = _call(client, "create_presentation_from_file", {"path": "report.html"})
    assert resp.json()["result"]["isError"] is True


def test_create_presentation_from_file_dotdot_escape_is_rejected(client, tmp_path, monkeypatch):
    root = tmp_path / "workspace"
    root.mkdir()
    outside = tmp_path / "outside.html"
    outside.write_text("<html>evil</html>")
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(root))

    escaping_path = str(root / ".." / "outside.html")
    resp = _call(client, "create_presentation_from_file", {"path": escaping_path})
    assert resp.json()["result"]["isError"] is True


def test_create_presentation_from_file_symlink_escape_is_rejected(client, tmp_path, monkeypatch):
    root = tmp_path / "workspace"
    root.mkdir()
    ...
    resp = _call(client, "create_presentation_from_file", {"path": str(link)})
    assert resp.json()["result"]["isError"] is True


def test_create_presentation_from_file_over_size_limit_is_rejected(client, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    limit = http_handler_mod._FROM_FILE_TEXT_LIMIT_BYTES
    f = tmp_path / "big.html"
    f.write_bytes(b"x" * (limit + 1))

    resp = _call(client, "create_presentation_from_file", {"path": str(f)})
    body = resp.json()
    assert body["result"]["isError"] is True
...
def test_create_presentation_from_file_missing_file_is_an_error(client, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    missing = tmp_path / "nope.html"
    resp = _call(client, "create_presentation_from_file", {"path": str(missing)})
    body = resp.json()
    assert body["result"]["isError"] is True
    assert "nope.html" in body["result"]["content"][0]["text"]


def test_create_presentation_from_file_image_goes_through_show_image_path(client, store, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    img = tmp_path / "shot.png"
    img.write_bytes(b"\x89PNG\r\n\x1a\nfakepngbytes")

    _call(client, "show_image", {"path": str(img), "title": "Screenshot", "id": "img-a"})
    _call(client, "create_presentation_from_file", {"path": str(img), "title": "Screenshot", "id": "img-b"})
...
def test_create_presentation_from_file_upserts_by_id(client, store, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    f1 = tmp_path / "v1.html"
    f1.write_text("<p>v1</p>")
    f2 = tmp_path / "v2.html"
    f2.write_text("<p>v2</p>")

    _call(client, "create_presentation_from_file", {"path": str(f1), "id": "same-id", "title": "V1"})
    resp = _call(client, "create_presentation_from_file", {"path": str(f2), "id": "same-id", "title": "V2"})
    assert resp.json()["result"]["isError"] is False

    listed = _call(client, "list_presentations")
    assert listed.json()["result"]["content"][0]["text"].count("same-id") == 1
    assert store.get("same-id").html == "<p>v2</p>"


def test_create_presentation_from_file_markdown_falls_back_to_preformatted_dark_theme(client, store, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    f = tmp_path / "notes.md"
    f.write_text("# Title\n\nSome *notes*.")

    resp = _call(client, "create_presentation_from_file", {"path": str(f), "id": "from-file-md"})
    assert resp.json()["result"]["isError"] is False
    html = store.get("from-file-md").html
    assert "#0d1117" in html  # this app's dark theme
    assert "# Title" in html  # raw markdown text preserved, not translated into a real <h1>


def test_create_presentation_from_file_invalid_utf8_is_replaced_not_fatal(client, store, tmp_path, monkeypatch):
    monkeypatch.setenv("AW_WORKSPACE_CONTAINER_DIR", str(tmp_path))
    f = tmp_path / "bad.html"
    f.write_bytes(b"<html><body>\xff\xfeBroken</body></html>")

    resp = _call(client, "create_presentation_from_file", {"path": str(f), "id": "bad-utf8"})
    assert resp.json()["result"]["isError"] is False
    assert "�" in store.get("bad-utf8").html
